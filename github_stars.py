...
class GitHubStars(object):
    ...
    THRESHOLD = 1 << 25  # when the stars probe exceeds this limit, we stop

    def __init__(self, login_or_token, password=None, **kwargs):
        self._start_index = kwargs.pop("start", 20)
    ...
    def search_query(
        ...
        if min_size and max_size:
            search_query += f"size:{min_size}..{max_size} "
        elif min_size:
            search_query += f"size:>{min_size} "
        elif max_size:
            search_query += f"size:<{max_size} "
        if language:
            search_query += f"language:{language} "
        self.additional_query = search_query
    ...
    def make_plan(self):
        ...
        while True:
            try:
                query = f"stars:{start}..{start + offset} {self.additional_query}"
                length = self._api.search_repositories(query).totalCount
    ...
    def fetch(self, plan):
        ...
        for i, p in enumerate(plan):
            ...
            while not success:
                try:
                    search_query = f"stars:{p[0]}..{p[1]} {self.additional_query}"
                    query = self._api.search_repositories(search_query, sort="stars", order="asc")
                    repos.extend(query)
                    success = True
                except RateLimitExceededException:
                    print("Hit rate limit, sleeping 60 seconds...")
                ...
                while not success:
                    try:
                        ...
                        query = self._api.search_repositories(
                            search_query, sort="stars", order="desc")
