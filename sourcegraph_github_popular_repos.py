...
SERVICE_UNAVAILABLE = b'error: 503 Service Unavailable\n\n'
            ...
            try:
                ...
                if stderr:
                    if TOO_MANY_REQUESTS == stderr:
                        ...
                        await asyncio.sleep(time_to_sleep)
                        continue
                    elif SERVICE_UNAVAILABLE in stderr:
                        time_to_sleep = random.uniform(MIN_TIMEOUT, MAX_TIMEOUT)
                        await asyncio.sleep(time_to_sleep)
                        continue
                    else:
...
async def main(folder='', repos_to_scan=None|set[str], language='python'):
    ...
    sbom_dict_filename = f'sourcegraph_gh_popular_repos_{language}.json'
    sbom_dict_filename = os.path.join(popular_repos_folder, sbom_dict_filename)
